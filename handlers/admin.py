...
# Импортируем PLANS из payment.py
...
def plans_list_kb():
    ...
    for plan_id, plan in PLANS.items():
        if plan_id == "trial":
            kb.button(
                text=f"{plan['emoji']} {plan['name']} (бесплатно)",
                callback_data=f"admin_edit_{plan_id}",
            )
        else:
            kb.button(
...
def plan_fields_kb(plan_id: str):
    kb = InlineKeyboardBuilder()
    plan = PLANS[plan_id]
    kb.button(text=f"📅 Дней: {plan['days']}", callback_data=f"admin_field_{plan_id}_days")
    if plan_id != "trial":
        kb.button(text=f"💳 Цена ₽: {plan['price_rub']}", callback_data=f"admin_field_{plan_id}_price_rub")
        kb.button(text=f"₿ Цена USDT: {plan['price_usdt']}", callback_data=f"admin_field_{plan_id}_price_usdt")
    kb.button(text=f"📝 Название: {plan['name']}", callback_data=f"admin_field_{plan_id}_name")
...
async def admin_plans(callback: CallbackQuery):
    ...
    await callback.message.edit_text(text, reply_markup=plans_list_kb(), parse_mode="HTML")
...
async def admin_edit_plan(callback: CallbackQuery):
    ...
    plan_id = callback.data.replace("admin_edit_", "")
    ...
    if plan_id != "trial":
        text += (
            ...
            f"₿ Цена USDT: <b>{plan['price_usdt']}</b>\n"
        )
    text += "\nВыбери что изменить:"
...
async def admin_field(callback: CallbackQuery, state: FSMContext):
    ...
    # admin_field_{plan_id}_{field}
    parts = callback.data.replace("admin_field_", "").split("_")
    # plan_id может быть "1m", "3m", "6m", "1y", "trial"
    field = parts[-1]
    ...
    kb = InlineKeyboardBuilder()
    kb.button(text="❌ Отмена", callback_data=f"admin_edit_{plan_id}")
...
async def process_new_value(message: Message, state: FSMContext):
    ...
    try:
        ...
        if field == "days" or field == "price_rub":
        ...
        kb = InlineKeyboardBuilder()
        kb.button(text="◀️ К тарифу", callback_data=f"admin_edit_{plan_id}")
...
async def admin_stats(callback: CallbackQuery, session):
    ...
    revenue_rub = await session.scalar(
        select(func.sum(Payment.amount)).where(
            Payment.status == "paid", Payment.currency == "RUB"
        )
    ) or 0
